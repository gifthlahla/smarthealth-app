...
class ClaimDetailScreen extends ConsumerWidget {
  ...
  Widget _buildClaimDetail(
      BuildContext context, WidgetRef ref, ClaimModel claim, bool isDark) {
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDetailCard(context, isDark, [
            _detailRow(context, 'Claim ID', claim.id.substring(0, 8).toUpperCase(), isDark),
            _detailRow(context, 'Type', claim.claimType.displayName, isDark),
            _detailRow(context, 'Service Date',
                DateFormat('MMMM dd, yyyy').format(claim.serviceDate), isDark),
            _detailRow(context, 'Submitted',
                DateFormat('MMMM dd, yyyy • h:mm a').format(claim.createdAt), isDark),
            _detailRow(context, 'Last Updated',
                DateFormat('MMMM dd, yyyy • h:mm a').format(claim.updatedAt), isDark),
          ]),
          ...
          if (claim.documents != null && claim.documents!.isNotEmpty) ...[
            const SizedBox(height: 24),
            _buildSectionTitle(context, 'Attached Documents (${claim.documents!.length})'),
            const SizedBox(height: 12),
            ...claim.documents!.map((doc) => _buildDocumentTile(context, doc, isDark)),
          ],
  ...
  Widget _buildDocumentTile(
      BuildContext context, ClaimDocumentModel doc, bool isDark) {
  ...
  Widget _buildStatusTimeline(
      BuildContext context, ClaimStatus currentStatus, bool isDark) {
